module DQMC_Phy0
    ...
    type Phy0
        ...
        ! Scalar array
        real(wp), pointer :: meas(:, :)                    ! Scalar variables
        real(wp), pointer :: sign(:, :)                    ! Scalar variables
        ...
        subroutine DQMC_Phy0_GetErr(P0)
            ...
            if (nproc == 1) then
                ...
                do i = P0_SGNUP, P0_SGNDN
                    data = P0%sign(i, 1:n)
                    call DQMC_JackKnife(n, P0%sign(i, avg), P0%sign(i, err), data, y, sgn, sum_sgn)
                end do
                
                data = P0%sign(P0_SGN, 1:n)
                call DQMC_JackKnife(n, P0%sign(P0_SGN, avg), P0%sign(P0_SGN, err), data, y, sgn, sum_sgn)
                
                ! Average other single terms
                do i = 1, P0%nMeas
                    data = P0%meas(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%meas(i, avg), P0%meas(i, err), data, y, sgn, sum_sgn)
                end do
                ...
                do i = 1, P0%nClass
                    data =  P0%Gf_up(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%Gf_up(i, avg), P0%Gf_up(i, err), data, y, sgn, sum_sgn)
                end do
                
                ! Down Green function
                do i = 1, P0%nClass
                    data =  P0%Gf_dn(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%Gf_dn(i, avg), P0%Gf_dn(i, err), data, y, sgn, sum_sgn)
                end do
       
                ! Average correlated Density 
                do i = 1, P0%nClass
                    data = P0%Den0(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%Den0(i, avg), P0%Den0(i, err), data, y, sgn, sum_sgn)
                end do
                
                do i = 1, P0%nClass
                    data = P0%Den1(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%Den1(i, avg), P0%Den1(i, err), data, y, sgn, sum_sgn)
                end do
       
                ! Average spin
                do i = 1, P0%nClass
                    data = P0%SpinXX(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%SpinXX(i, avg), P0%SpinXX(i, err), data, y, sgn, sum_sgn)
                end do
       
                do i = 1, P0%nClass
                    data = P0%SpinZZ(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%SpinZZ(i, avg), P0%SpinZZ(i, err), data, y, sgn, sum_sgn)
                end do

                do i = 1, P0%nClass
                    P0%AveSpin(i, 1:n) = ( P0%SpinZZ(i, 1:n) + 2.d0*P0%SpinXX(i, 1:n) ) / 3.d0
                    data = P0%AveSpin(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%AveSpin(i, avg), P0%AveSpin(i, err), data, y, sgn, sum_sgn)
                end do

                ! Pairing
                do i = 1, P0%nClass
                    data = P0%Pair(i, 1:n)
                    call DQMC_SignJackKnife(n, P0%Pair(i, avg), P0%Pair(i, err), data, y, sgn, sum_sgn)
                end do
                ...
                P0%meas(P0_CHIT, 1:avg) = P0%meas(P0_CHIT, 1:avg) - P0%n *  P0%beta**2 * P0%meas(P0_ENERGY, 1:avg) * P0%meas(P0_DENSITY, 1:avg)
...
#               ifdef _QMC_MPI
                    ...
                    ! Average sign
                    call mpi_allreduce(P0%sign(:, 1)   , P0%sign   (:, avg), 3, mpi_double, mpi_sum, mpi_comm_world, mpi_err)

                    ! Average properties
                    call mpi_allreduce(P0%AllProp(:, 1), P0%AllProp(:, avg), n, mpi_double, mpi_sum, mpi_comm_world, mpi_err)
                    ...
                    ! Compute error
                    call mpi_allreduce(P0%AllProp(:, 1)**2, P0%AllProp(:, err), n, mpi_double, mpi_sum, mpi_comm_world, mpi_err)
                    ...
                    call mpi_allreduce(P0%sign(:,1)**2, P0%sign(:,err), 3, mpi_double, mpi_sum, mpi_comm_world, mpi_err)
                    P0%sign(:, err) = P0%sign(:, err) / dble(nproc) - P0%sign(:, avg)**2 
                    P0%sign(:, err) = sqrt(P0%sign(:, err) * dble(nproc-1))

                    P0%meas(P0_CHIT,avg) = P0%meas(P0_CHIT,avg) - P0%n * P0%beta**2 * P0%meas(P0_ENERGY,avg) * P0%meas(P0_DENSITY,avg)
...
#               endif
        ...
        subroutine DQMC_Phy0_Meas(n, P0, G_up, G_dn, U, mu_up, mu_dn, t_up, t_dn, sgnup, sgndn, S)
            ...
            do i = 1, n
                ...
                P0%meas(P0_NUD, tmp) = P0%meas(P0_NUD, tmp) + P0%up(i) * P0%dn(i) * U(S%Map(i))

                !======================================================!
                ! Double occupancy P0%up(i) * P0%dn(i)
                !======================================================!
                P0%meas(P0_double_occupancy, tmp) = P0%meas(P0_double_occupancy, tmp) + P0%up(i) * P0%dn(i)

                !=====================================================================!
                ! Potential energy (P0%up(i)-0.5d0) * (P0%dn(i)-0.5d0) * U(S%Map(i))
                !=====================================================================!
                P0%meas(P0_potential_energy, tmp) = P0%meas(P0_potential_energy, tmp) + (P0%up(i) - 0.5d0) * (P0%dn(i) - 0.5d0) * U(S%Map(i))
            end do
            ...
            do i = 1, n  ! for each column
                do j = start(i), start(i + 1)-1 ! for each nonzero elements
                    P0%meas(P0_KE, tmp)             = P0%meas(P0_KE, tmp) + t_up(A(j)) * G_up(r(j), i) + t_dn(A(j)) * G_dn(r(j), i)
                    P0%meas(P0_hopping_energy, tmp) = P0%meas(P0_hopping_energy, tmp) + t_up(A(j)) * G_up(r(j), i) + t_dn(A(j)) * G_dn(r(j), i)
                end do
                P0%meas(P0_KE, tmp) = P0%meas(P0_KE, tmp) - (mu_up(S%Map(i)) + 0.5d0 * U(S%map(i))) * P0%up(i) - (mu_dn(S%Map(i)) + 0.5d0 * U(S%map(i))) * P0%dn(i)            
            end do
            ...
            !=================================================================! 
            P0%meas(P0_magnetization_squared, tmp) = 0.25d0 * (P0%meas(P0_density, tmp) - 2 * P0%meas(P0_double_occupancy, tmp))

            !=================================================================!
            ! Total energy = hopping energy + potential energy - 
            ! - sum_{i \sigma} (\mu_{up i \sigma} n_{dn i \sigma})
            !=================================================================!
            P0%meas(P0_ENERGY, tmp) = P0%meas(P0_hopping_energy, tmp) + P0%meas(P0_potential_energy, tmp)
            do i = 1, n
                P0%meas(P0_ENERGY, tmp) = P0%meas(P0_ENERGY, tmp) - (mu_up(S%Map(i)) * P0%up(i) + mu_dn(S%Map(i)) * P0%dn(i))
            enddo
            ...
            do i = 1, n
                do j = 1, n 
                    P0%meas(P0_CV, tmp) = P0%meas(P0_CV, tmp) + (h_up(i, j) - A_up(i, j)) * A_up(j, i) + (h_dn(i, j) - A_dn(i, j)) * A_dn(j, i)
                enddo
            enddo
            ...
            !=====================!
            if (P0%compSAF) then
                P0%meas(P0_SAF, tmp)  = TWO*n - P0%meas(P0_NUP, tmp) - P0%meas(P0_NDN, tmp)
            ...
            do i = 1, n
                do j = 1, n
                    ...
                    var3 = G_up(i,i) * G_up(j,j) + G_dn(i,i) * G_dn(j,j) - TWO * G_up(i,i) * G_dn(j,j) - var1
                    ...
                    P0%Den0(k, tmp)  = P0%Den0(k, tmp) + P0%up(i)*P0%up(j) + P0%dn(i)*P0%dn(j) - var1
...
#               ifdef _QMC_MPI
                    call mpi_allreduce(P0%AllPropEigVal(:, 1)**2, P0%AllPropEigVal(:,err), n, mpi_double, mpi_sum, mpi_comm_world, i)
                    call mpi_allreduce(P0%AllPropFT(:, 1)**2, P0%AllPropFT(:,err), m, mpi_double, mpi_sum, mpi_comm_world, i)
#               endif
        ...
        subroutine DQMC_Phy0_PrintFT(P0, na, nkt, nkg, OPT)
            ...
            call DQMC_Print_RealArray(0, nakt*(na+1)/2, "FT of Ave Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(IGFUP):P0%IARRFT(IGFUP+1)-1, :)
            call DQMC_Print_RealArray(0, nakt*(na+1)/2, "FT of Up Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(IGFDN):P0%IARRFT(IGFDN+1)-1, :)
            call DQMC_Print_RealArray(0, nakt*(na+1)/2, "FT of Dn Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)
            ...
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of Density-density correlation fn: (up-up)", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(IDEN1):P0%IARRFT(IDEN1+1)-1, :)
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of Density-density correlation fn: (up-dn)", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(ISPXX):P0%IARRFT(ISPXX+1)-1, :)
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of XX spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(ISPZZ):P0%IARRFT(ISPZZ+1)-1, :)
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of ZZ spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(IAVSP):P0%IARRFT(IAVSP+1)-1, :)
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of Average spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

            FTptr => P0%AllPropFT(P0%IARRFT(IPAIR):P0%IARRFT(IPAIR+1)-1, :)
            call DQMC_Print_RealArray(0, nakg*(na+1)/2, "FT of Pairing correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)
            ...
            if (na > 1) then
                ...
                call DQMC_Print_RealArray(0, nakt, "Eigenvalues of Ave Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(IGFUP):P0%IARREV(IGFUP+1)-1, :)
                call DQMC_Print_RealArray(0, nakt, "Eigenvalues of Up Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(IGFDN):P0%IARREV(IGFDN+1)-1, :)
                call DQMC_Print_RealArray(0, nakt, "Eigenvalues of Dn Equal t Green's function:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)
                ...
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of Density-density correlation fn: (up-up)", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(IDEN1):P0%IARREV(IDEN1+1)-1, :)
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of Density-density correlation fn: (up-dn)", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(ISPXX):P0%IARREV(ISPXX+1)-1, :)
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of XX spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(ISPZZ):P0%IARREV(ISPZZ+1)-1, :)
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of ZZ spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(IAVSP):P0%IARREV(IAVSP+1)-1, :)
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of Average spin correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                FTptr => P0%AllPropEigVal(P0%IARREV(IPAIR):P0%IARREV(IPAIR+1)-1, :)
                call DQMC_Print_RealArray(0, nakg, "Eigenvalues of Pairing correlation fn:", clabel, FTptr(:, avg:avg), FTptr(:, err:err), OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IGFUN)
                call DQMC_Print_EigenMode(na, nkt, "Eigenmodes of Ave Equal t Green's function (Natural orbitals):", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IGFUP)
                call DQMC_Print_EigenMode(na, nkt, "Eigenmodes of Up Equal t Green's function (Natural orbitals):", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IGFDN)
                call DQMC_Print_EigenMode(na, nkt, "Eigenmodes of Down Equal t Green's function (Natural orbitals):", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IDEN0)
                call DQMC_Print_EigenMode(na, nkg,"Eigenmodes of Density-density correlation fn: (up-up)", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IDEN1)
                call DQMC_Print_EigenMode(na, nkg, "Eigenmodes of Density-density correlation fn: (up-dn)", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,ISPXX)
                call DQMC_Print_EigenMode(na, nkg, "Eigenmodes of XX-Spin correlation fn: ", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,ISPZZ)
                call DQMC_Print_EigenMode(na, nkg, "Eigenmodes of ZZ-Spin correlation fn: ", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IAVSP)
                call DQMC_Print_EigenMode(na, nkg, "Eigenmodes of Average Spin correlation fn: ", Nmptr, OPT)

                Nmptr => P0%AllPropEigVec(:,:,:,IPAIR)
                call DQMC_Print_EigenMode(na, nkg, "Eigenmodes of Pairing correlation fn: ", Nmptr, OPT)
