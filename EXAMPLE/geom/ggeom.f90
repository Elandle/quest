program dqmc_ggeom
        ...
        do i = 1, nBin
            do j = 1, nIter / qmc_sim%aggr_size
                ...
                if (comp_tdm > 0) then
                    ...
                    call DQMC_Hub_FullMeas(Hub, tau%nb, tau%A_up, tau%A_dn, tau%sgnup, tau%sgndn)
    ...
    ! I was getting errors when running multithreaded and tdm = 0 (but not multithreaded and tdm = 1)
    ! Both seem to run fine commenting this out.
    ! Up for eventual deletion if this persists.
    ! if (nproc > 1) then
        ! deallocate(ptm)
        ! deallocate(ptau)
    ! endif
