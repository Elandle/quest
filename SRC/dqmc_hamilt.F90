module DQMC_HAMILT
    ...
    type :: Hamiltonian_t
        ...
        integer, pointer    :: tsite(:), Usite(:), Jsite(:)          ! Sites having a non-zero J,U,t (0:nsites-1)
        integer, pointer    :: tneig(:,:), Uneig(:,:), Jneig(:,:)    ! Sites neighboring each site   (0:nsites-1,nsites)  
        integer, pointer    :: tckb(:,:)                             ! Pair of sites associated to each hopping throughout the system
        real*8              :: mu_up, mu_dn                          ! chemical potential
        real*8, pointer     :: Uv(:,:), Jv(:,:)                      ! value of U and J for each pair (0:nsites-1, 0:nsites-1)                  
        real*8, pointer     :: Uvalue(:), muupvalue(:), mudnvalue(:) ! values of U and mu inequivalent by symmetry (nlocclass)
        complex*16, pointer :: hopup(:,:), hopdn(:,:)                ! value of t for each pair (0:nsites-1, 0:nsites-1)
        complex*16, pointer :: tupvalue(:), tdnvalue(:)              ! values of t inequivalent by symmetry (nhopclass)
        ...
        integer             :: nplink                                ! number of primitive links
    ...
    contains
        ...
        subroutine count_hop_class(lattice, hamilt)
            !
            ! Construct a list of only hopping classes. 
            ! These are a subset of all the distance classes for which the hopping is non zero.
            ! Find which hoppings are equivalent by symmetry.
            ! nhopsite(iat,iclass) returns how many hoppings in class iclass iat has.
            ! ordered_neig(iat,ineig) returns the ineig-th neighbor of iat.
            ! Neighbors are here ordered according to classes.
            ! So if nhopsite(2,1)=3 the first three neigbors of 2 listed in ordered_neig are those belonging to class 1.
            ! tneig has a similar content but neighbors are not ordered.
            !
        ...
        subroutine count_local_classes(lattice,hamilt)
            !
            ! Construct a list of only local classes i.e. the subset of the distance classes defined on the same site.
            ! Note that we use the symmetry instead of the value to group sites together.
            ! This is because local classes refers to both U and mu and two sites may have same value of U but different mu.
            ! If equivalent by symmetry, the two sites have, however, necessarily same u and mu.
            !
            type(Hamiltonian_t), intent(inout) :: hamilt
        ...
        subroutine group_hopping(hamilt, n, nt, tmap, tupvalue, tdnvalue)
            !
            ! Assign to every non-zero hopping a class based on its value.
            ! Equal hopping matrix elements belong to the same class : tmap(i,j)=it.
            ...
            if (maxval(hamilt%tnneig) > 0) then
                ...
                do is = 0, n-1
                    do jn = 1, hamilt%tnneig(is)
                        ...
                        do it = 1, nt
                            if (abs(tup-tupvalue(it)) < 1.d-6 .and. abs(tdn-tdnvalue(it)) < 1.d-6) exit
                        enddo
                        if (it .eq. nt+1) then
                            nt = it
                            tupvalue(it) = tup
