module DQMC_RECLATT
    ...
    contains
        ...
        subroutine construct_recip_lattice(recip_lattice)
            integer                :: nkx, nky, nkz, ndim, ikx, iky, ikz, ikv(rdim), ncubex, ncubey, ncubez, ilist, nedge, nfound, nkpts, i, j
            ...
            ilist = 1
            klist(ilist, :) = recip_lattice%ktwist(:)
            nedge = 0
            if (ndim > 0) then
                do 
        ...
        subroutine DQMC_init_kmate(reclatt, nmom, ksum)
            ...
            do im = 1, reclatt%nmomenta
                ...
                do ik = 1, reclatt%nkpts
                    jk = reclatt%kmate(ik, im)
                    write(*,'(3(i3, 3f10.5,10x))') ik, (reclatt%klist(ik, i), i=1, rdim), jk, (reclatt%klist(jk, i), i=1, rdim)
                enddo
                write(*, *) '==============================================================='
            enddo

        end subroutine DQMC_init_kmate
