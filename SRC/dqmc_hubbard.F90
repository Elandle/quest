...
#   endif
    ...
    contains
        ...
        subroutine DQMC_Hub_Init(Hub, U, t_up, t_dn, mu_up, mu_dn, L, n_t, n_U, n_mu, dtau, &
            ...
            ! Purpose
            ! ==============
            !    This subroutine initializes Hubbard model.
            !
            ! Pre-assumption
            ! ==============
            !    DQMC_default should be called before this.
...
#           endif
            ...
            if (HSF == HSF_FROM_FILE .or. HSF == HSF_RESTORE) then
                ...
                if (lex) then
                    ...
                    call DQMC_Warning("HSF file does not exist. Use random generated values.", HSF)
                    HSF = HSF_RANDOM_GEN
                end if
            end if
            ...
            ! Initialize G
            call DQMC_GFun_Init(n, L, Hub%G_up, Hub%V_up,  Hub%WS, nWrap, difflim, errrate, GMAT_UP, ssxx, fixw)

            ! for positive U or H_dn/=H_up we need to construct G_dn implicitly
            if (Hub%comp_dn .or. .not. Hub%neg_u) then
                call DQMC_GFun_Init(n, L, Hub%G_dn, Hub%V_dn,  Hub%WS, nWrap, difflim, errrate, GMAT_DN, ssxx, fixw)
            else
        ...
        subroutine DQMC_Hub_OutputParam(Hub, OPT)
            ...
            if (Hub%outputHSF) then
                ...
                if (lex) then
                    ...
                    if (qmc_sim%rank == qmc_sim%aggr_root) call DQMC_Warning("HSF output file is not initialized.", 1)
                end if
            end if
...
#                       endif
        ...
        subroutine DQMC_Hub_Sweep2_Cont(Hub, numTry)
            ...
            ! Compute the Green's matrix and the sign
#           if defined(DQMC_ASQRD)
                call cpp_gfun_computeg(Hub%G_up%cpp_data, L, Hub%G_up%sgn, Hub%G_up%G, Hub%V_up, Hub%SB_up%B%B, Hub%SB_up%nOrth, det_up)
#           else
                call DQMC_ComputeG(L, n, Hub%G_up%sgn, Hub%G_up%G, Hub%V_up, Hub%SB_up, Hub%G_up%pvt, .true., det_up, HUb%G_up%sxx)
#           endif
            if (compute_dn) then
#               if defined(DQMC_ASQRD)
                    call cpp_gfun_computeg(Hub%G_dn%cpp_data, L, Hub%G_dn%sgn, Hub%G_dn%G, Hub%V_dn, Hub%SB_dn%B%B, Hub%SB_dn%nOrth, det_dn)
#               else
                    call DQMC_ComputeG(L, n, Hub%G_dn%sgn, Hub%G_dn%G, Hub%V_dn, Hub%SB_dn, Hub%G_dn%pvt, .true., det_dn, Hub%G_dn%sxx)
#               endif
            else
                det_dn = det_up
                Hub%G_dn%sgn = Hub%G_up%sgn
...
#                   endif
                ...
                ! Compute G with new configuration
#               if defined(DQMC_ASQRD)
                    call cpp_gfun_computeg(Hub%G_up%cpp_data, L, n, Hub%G_up%sgn, Hub%G_up%G, Hub%V_up, Hub%SB_up%B%B, Hub%SB_up%L, Hub%SB_up%nOrth, new_up)
#               else
                    call DQMC_ComputeG(L, n, Hub%G_up%sgn, Hub%G_up%G, Hub%V_up, Hub%SB_up, Hub%G_up%pvt, .true., new_up, Hub%G_up%sxx)
#               endif
                if (compute_dn) then
#                   if defined(DQMC_ASQRD)
                        call cpp_gfun_computeg(Hub%G_up%cpp_data, L, n, Hub%G_dn%sgn, Hub%G_dn%G, Hub%V_dn, Hub%SB_dn%B%B, Hub%SB_dn%L, Hub%SB_dn%nOrth, new_dn)
#                   else
                        call DQMC_ComputeG(L, n, Hub%G_dn%sgn, Hub%G_dn%G, Hub%V_dn, Hub%SB_dn, Hub%G_dn%pvt, .true., new_dn, Hub%G_up%sxx)
#                   endif
                else
                    new_dn = new_up
                    Hub%G_dn%sgn =  Hub%G_up%sgn
...
#                       endif
        ...
        subroutine DQMC_Hub_FullMeas(Hub, nnb, A_up, A_dn, sgn_up, sgn_dn)
            ...
            do it = 0, nb-1
                ...
                call DQMC_Phy0_Meas(Hub%n, Hub%P0, G_up_local%GS, G_dn_local%GS, Hub%U, Hub%mu_up, Hub%mu_up, Hub%t_up, Hub%t_dn, sgn_up, sgn_dn, Hub%S)
        ...
        subroutine DQMC_Hub_Meas(Hub, slice)
            ...
            ! Basic measurement
            call DQMC_Phy0_Meas(Hub%n, Hub%P0, G_up_local%GS, G_dn_local%GS, Hub%U, Hub%mu_up, Hub%mu_dn, Hub%t_up, Hub%t_dn, sgn_up, sgn_dn, Hub%S)

            if (Hub%meas2) then
                ! Pair measurement
                call DQMC_Phy2_Meas(Hub%n, Hub%P2%M1, Hub%P2%M2, Hub%P2, Hub%S%B, G_up_local%GS, G_dn_local%GS, sgn_up*sgn_dn)
            end if
