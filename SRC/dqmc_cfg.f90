!> \file
!! @brief Holds the dqmc_cfg module.
!! This module contains subroutines and datatypes for reading and holding the data of input config (cfg) files.
...
module DQMC_Cfg
    ...
    !>
    !! Must be of length \p N_Param. See \p PARAM_DVAL for the default value of a parameter, which is used if not specified on input.
    !!
    !> \b Parameter \b names:
    !! \verbatim
    ...
    type Param
        !> Uniquely identifies the parameter that an instance of this data type refers to.
        !> For a given parameter, this value is determined by that parameters index in \p PARAM_NAME.
        !> Lies between 1 and \p N_Param (inclusive).
        integer                 :: id           ! hashcode from name
        !> The name of the parameter that an instance of this data type refers to.
        !> All possible parameter names are stored in \p PARAM_NAME.
        !> More specifically, \p PARAM_NAME(\p id) = \p pname.
        character(len=slen)     :: pname        ! parameter name
        !> One of \p TYPE_STRING, \p TYPE_INTEGER, or \p TYPE_REAL.
        !> Determined by \p PARAM_TYPE(\p id)
        integer                 :: ptype        ! type of parameter
        !> .true. if the values of the parameter are stored as an array, .false. otherwise.
        !> Determined by \p PARAM_ARRAY(\p id)
    ...
    !> Data type for holding the configuration (config/cfg) of a simulation.
    !> Implemented as a linked list of \p Param data types.
    type config
    ...
    contains
        ...
        !---------------------------------------------------------------------!
        !> \brief Reads in parameters from a .def input file to a config (cfg) data type.
        !> \param[inout] cfg (\p config) config data type to store read in parameters to.
        !> \param[in] IPT (\p integer) unit number of opened .def input file.
        subroutine DQMC_Read_Def(cfg, IPT)
            !
            ! Purpose
            ! =======
            !    This subroutine reads in parameters from a config file.
            !
            ! Arguments
            ! =========
            !
                ...
                if (len_trim(str) .gt. 0) then
                    ...
                    if (pos .ne. 0) then
                        ...
                        if (id .gt. 0) then
                            ...
                            if (curr%isArray) then
                                ! array case
                                if (curr%ptype .eq. TYPE_REAL .or. curr%ptype .eq. TYPE_INTEGER) then
                                    j = 1
        ...
        subroutine DQMC_Config_GetI(cfg, name, value)
            ...
            if (id .gt. 0) then
                if (.not. cfg%record(id)%isSet) then
                    call DQMC_Warning(name//" wasn't initialized, used default setting.",1)
