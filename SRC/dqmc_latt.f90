module DQMC_LATT
    ...
    contains
        ...
        subroutine construct_lattice(lattice, SOP)
            ...
            ! Find lattice points inside the supercell without counting the ones on edge twice
            icount = -1
        ...
        subroutine assign_phase(lattice)
            ...
            real*8              :: rpc(rdim, rdim), volume_ph, inv(rdim, rdim), projph(rdim, rdim), diff(rdim), projph2(rdim), ainv(rdim, rdim)
            character*50        :: string
            character*3         :: olab1
