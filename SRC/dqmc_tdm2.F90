...
#   include "dqmc_include.h"
    ...
    contains
        ...
        subroutine DQMC_TDM2_Avg(T2, gnl, idx, factor)
            ...
            ! For suscetibility
            do j = 1, L+1
                call DQMC_Wave_Avg(nB, nWave, T2%Wave, T2%T, T2%Apair(:,:,j), T2%Bpair(1:nWave,j,idx), factor, T2%ldt)
            end do
            ...
            do ti = 1, L+1
                do i = 1, T2%n
                    do inbr = 1, nB
                        do jnbr = 1, nB
                            susnl(jnbr,inbr,ti) = susnl(jnbr,inbr,ti) + gnl(D(jnbr, inbr, i), ti) * gnl(SD(i,1),ti)
                        end do
                    end do
                end do
            end do
            ...
            ! non vertex FT 
            do j = 1, L+1
                call DQMC_Wave_Avg(nB, nWave, T2%Wave, T2%T, susnl(:,:,j), T2%Npair(1:nWave,j,idx), ONE, T2%ldt)
            end do
