module DQMC_SYMM
    ...
    contains
        ...
        subroutine apply_point_symm(label, point, axis, theta, set, new_set, nset, reciprocal)
            ...
            real*8 trans_set(3, nset), d, a1, a2, rot1(3, 3), rot2(3, 3), rot3(3, 3), globrot(3, 3), cost, sint, dpoint(3), dset(3, nset), dset2(nset, 3)
            character*1 label
        ...
        subroutine map_symm_recip_lattice(SymmOp, recip_lattice, applytwist)
            ...
            real*8                              :: diff(rdim), projsc(rdim), invkc(rdim, rdim), newklist(recip_lattice%nkpts, rdim), zerovec(rdim)
            ...
            do j = 1, SymmOp%nsymm
                ...
                call apply_point_symm(symmlabel(i), symmpoint(:, i), symmaxis(:, i), symmangle(i), recip_lattice%klist, newklist, recip_lattice%nkpts, .true.)
            ...
            if (msymm /= SymmOp%nsymm .and. applytwist) then
                ...
                allocate(tmp_symm(0:nsites-1, SymmOp%nsymm), tmp_symm_k(recip_lattice%nkpts,nsymm), tmp_valid(SymmOp%nsymm))
            ...
            ! Add the additional symmetry time reversal symmetry when "I" is not a symmetry operation specified in input
            if (SymmOp%addTimeRev) then
                mapped(:) = .false.
                zerovec(1:3) = (/0.d0, 0.d0, 0.d0/)
                call apply_point_symm('I', zerovec, zerovec, 0.d0, recip_lattice%klist, newklist, recip_lattice%nkpts, .true.)
                do ik = 1, recip_lattice%nkpts
                    ! Find which k-point, "jk", "ik" is mapped onto by symmetry "j" 
                    do jk = 1, recip_lattice%nkpts
                        ! For each k-point compute distance in unit of k-space unit cell
        ...
        subroutine map_symm_bonds(Bonds,SymmOp,Lattice)
            ...
            type(lattice_t), intent(in) :: Lattice
            integer :: nbclass, ibond, iat, jat, ntotpair, natom, nsites, bcl, newlabel, jbond, it, isymm, ndim
            ...
            if (Bonds%ntotbond /= ntotpair) then
                ...
                allocate(Bonds%bond_origin(ntotpair), Bonds%bond_target(ntotpair), Bonds%xxbond(3,ntotpair), Bonds%bond_label(ntotpair))
        ...
        subroutine construct_lattice_classes(SymmOp,lattice)
            use DQMC_LATT
            integer               :: i, it, ip, is, j, isymm, iclass, istart, csize, csizenew, itransl, jclass, idj, id, mclass, ip_transl, is_transl, ip2, is2, jstart, nclass, nsites, natom, nsymm, ntransl
                ...
                do iclass=1, nclass
                    ...
                    do 
                        ...
                        do id=istart, csize
                            ...
                            ! Find the transformed pair translated such that the firts site is inside the primitive cell
                        ...
                        enddo
                        ! if class size did not change we have found all the classes equivalent to "iclass"
        ...
        subroutine construct_recip_lattice_classes(SymmOp,recip_lattice,applytwist)
            use DQMC_RECLATT
            integer               :: i, ip, j, isymm, iclass, istart, csize, csizenew, jclass, idj, id, mclass, ip2, jstart, nsymm, nkpts
        ...
        subroutine construct_bond_classes(Bonds, SymmOp)
            ...
            type(bonds_t), intent(inout) :: Bonds
            integer :: ib, nclass, ntotbond, ntotbondsq, isymm, iclass, istart, csize, csizenew, id, bx, by, jclass, jstart, idj, mclass, jb, i, j
            integer, pointer :: myclass(:,:) 
            ...
            allocate(myclass(ntotbond, ntotbond), bond1(2, ntotbondsq), bond2(2, ntotbondsq), csizev(ntotbondsq))
            ...
            contains
                ...
                subroutine resize_class()
                    ...
                    if (csizenew > curr_csize) then
                        ...
                        deallocate(bond2)
                        allocate(bond2(csizenew, nclass))
                        bond2 = tmpbond
                        ! deallocate temp array
                        deallocate(tmpbond)
                    end if
        ...
        subroutine construct_pair_classes(Bonds, Pairs)
            ...
            ! bclass maps bond classes on pair classes. When 0 it means the bond class has yet to be mapped
            bclass = 0
            ! loop over all pairs
            do ip=1, np
