    ...
    contains
        ...
        subroutine DQMC_Phy2_Avg_Wave(P2, W)
            ...
            if (P2%compute) then
                ...
                call DQMC_Wave_Avg(P2%nb, P2%nWave, W, P2%T, P2%M2, P2%M3(1:P2%nWave,P2%idx), factor, P2%ldt)
        ...
        subroutine DQMC_Phy2_Avg_Symm(P2, S)
            ...
            if (P2%compute) then
                factor = ONE/P2%cnt       
                call DQMC_Pair_Symm(P2%M2, P2%M3(1:S%nclass_b,P2%idx), S%class_b, S%size_b, S%n_b, S%nclass_b, factor)
                P2%sgn(P2%idx) = P2%sgn(P2%idx) * factor
        ...
        subroutine DQMC_Wave_Avg(nb, nWave, W, T, I, O, factor, ldt)
            ...
            call blas_dgemm('N','N', nb, nWave, nb, factor, I, nb, W, nb, ZERO, T, ldt)
            do j = 1, nWave
        ...
        subroutine DQMC_Phy2_GetErr(P2)
            ...
            if (P2%compute) then
                ...
                if (nproc == 1) then
                    ...
                    do i = 1, m
                        data = P2%M3(i, 1:n)
                        call DQMC_SignJackKnife(n, P2%M3(i, avg), P2%M3(i, err), data, y, sgn, sum_sgn)
                    end do
...
#                   ifdef _QMC_MPI
                        ! Sum sign
                        call mpi_allreduce(P2%sgn(1), P2%sgn(avg), 1, mpi_double, mpi_sum, mpi_comm_world, n)

                        ! Sum properties
                        call mpi_allreduce(P2%M3(:,1), P2%M3(:,avg), m, mpi_double, mpi_sum, mpi_comm_world, n)
                        ...
                        !Compute error
                        call mpi_allreduce(P2%M3(:, 1)**2, P2%M3(:, err), m, mpi_double, mpi_sum, mpi_comm_world, n)
...
#                   ifdef _QMC_MPI
                        call mpi_allreduce(MX(:,1)**2, MX(:,err), j, mpi_double, mpi_sum, mpi_comm_world, k)
#                   endif
        ...
        subroutine DQMC_Phy2_Print(P2, wlabel, OPT)
            ...
            avg = P2%avg
            err = P2%err
            call DQMC_Print_RealArray(0, P2%nWave, "Pair-field correlation function - accumulated:", wlabel, P2%M3(:,avg:avg), P2%M3(:,err:err), OPT)
            end if
        ...
        end subroutine DQMC_phy2_WaveSymm
